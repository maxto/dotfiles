...
**Goal:** Add `bin/desk`, a script that builds the herdr layout preset — left 70% agent shell, right 30% split into top 40% bash and bottom 60% broot — idempotently in the current tab.

**Architecture:** herdr has no declarative startup layout; the layout is built imperatively over herdr's socket API (JSON out, parsed with `jq`). `bin/desk` resolves an anchor pane (the focused pane, or an explicit `PANE_ID` argument), collapses that pane's tab to a single pane, then performs two splits and launches broot. The explicit-argument path exists so an integration test can build the layout around a throwaway `--no-focus` workspace without stealing the user's focus.
...
- `bin/desk` must be **idempotent against the current tab**: collapse the tab to one pane, then rebuild. Re-running yields the same clean preset.
...
- Left and top-right panes stay plain bash shells; broot auto-launches bottom-right. The agent is launched manually by the user — `bin/desk` is agent-agnostic.
- `bin/desk` requires `jq` and `herdr`; it must error clearly (exit 1) if either is missing.

---

### Task 1: `bin/desk` layout script + live integration test + README

**Files:**
- Create: `bin/desk`
- Create: `tests/desk_test.sh`
- Modify: `README.md` (document the `desk` preset)

**Interfaces:**
- Consumes: a running herdr `0.7.2` server (the socket API), `jq`, `broot` — all present on the target machine.
- Produces: `bin/desk [PANE_ID]` — with no argument it builds around the focused pane; with a pane id it builds around that pane. `tests/desk_test.sh` — a live integration test that builds the preset in a throwaway workspace and asserts geometry/behavior.

> **Note on TDD here:** there is no headless herdr in CI; the "test" is a live integration test that drives the real herdr server but isolates itself in a throwaway `--no-focus` workspace it creates and removes, so it never disturbs the user's session. Write the test first, watch it fail (no `bin/desk`), then implement.
...
Run: `bash tests/desk_test.sh`
Expected: creates the throwaway workspace, then FAILs because `bin/desk` does not exist yet (`bash: .../bin/desk: No such file or directory`), so the geometry checks FAIL. The `trap` still removes the `desktest` workspace on exit. Confirm with `herdr workspace list` that no `desktest` workspace lingers.

- [ ] **Step 3: Write `bin/desk`**

```bash
#!/usr/bin/env bash
# Build the herdr "desk" layout preset in a tab:
...
# (re-run = reset). Usage: desk [PANE_ID]  (default: the focused pane).
set -euo pipefail
...
echo "desk: layout ready (agent | bash / files)"
```
...
```bash
chmod +x bin/desk
bash tests/desk_test.sh
```
Expected: every line prints `PASS:` and the script exits 0. The reported `~70%` and `~60%` values should land near 70 and 60. Then confirm no leftover workspace: `herdr workspace list` shows only the user's real workspace(s), no `desktest`.

- [ ] **Step 5: Document `desk` in README.md**
...
```bash
git add bin/desk tests/desk_test.sh README.md
git commit -m "feat: add bin/desk herdr layout preset (70/30 . 40/60) with live test"
```
...
`bin/desk` was proven correct by the automated geometry test in Task 1. Task 2 is the human confirming the preset looks and behaves right in their actual attached herdr — the one part a script cannot check.

**Files:** none (verification only).

**Interfaces:**
- Consumes: `bin/desk` from Task 1, on `PATH` via `dotfiles/bin` (installed in Phase 1).

- [ ] **Step 1: Confirm `desk` is on PATH**

Run (interactive shell): `command -v desk`
Expected: `/home/maxto/projects/personal/dotfiles/bin/desk`. (If empty, open a fresh shell so `shell/bashrc` is sourced.)

- [ ] **Step 2: Build the preset in a fresh herdr tab**

In herdr, open a new tab (single pane), then run: `desk`
Expected: the tab becomes three panes — a wide left pane, and a right column split into a smaller top and a larger bottom running broot. The command prints `desk: layout ready (agent | bash / files)`.
...
- Focus is on the left (agent) pane after `desk` finishes.

- [ ] **Step 4: Confirm idempotent reset**

From the left pane, run `desk` again.
...
No code change. If Steps 2–4 all pass, Phase 2 is done. If anything looks off (wrong proportions, broot not launching, focus wrong), report exactly what differed — the automated test in Task 1 should be extended to cover the gap before re-fixing `bin/desk`.
...
**Spec coverage:**
- `bin/desk` builds the 70/30 · 40/60 preset via the verified API sequence → Task 1 Step 3.
...
**Placeholder scan:** No TBD/TODO; `bin/desk`, the test, and the README block are complete and literal. `PANE_ID` is a documented optional argument, not a placeholder.

**Type/name consistency:** `anchor`/`right`/`bottom` pane variables and the JSON paths in the script match those asserted in the test and listed in Global Constraints. The test builds with an explicit anchor argument, exactly the code path `bin/desk` exposes.
