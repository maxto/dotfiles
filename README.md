...
- `shell/` — `bashrc`, sourced from `~/.bashrc` (prompt init, eza aliases, `herdrh`)
...
`brew outdated` previews what would change. After upgrading **herdr**, stop the
server of every session so the new binary is picked up — each session runs its
own, and the next launch starts them again:

```bash
herdr server stop                        # the default session
herdr --session <name> server stop       # once per named session
```

`oh-my-posh` can also self-update in place with `oh-my-posh upgrade`.

## herdr sessions

`herdr` on its own opens the default persistent session. If that session already
holds workspaces it restores them and ignores the directory you launched from —
the launch directory only matters for a session that has none. To keep a
project's panes separate from everything else, name the session after it:

```bash
herdr --session dotfiles    # creates it if missing, attaches if it already exists
```

`shell/bashrc` wraps that in `herdrh`, which names the session after the git
repository you are in (any subdirectory of a repo reaches the same session) and
falls back to the current folder outside one. `herdrh <name>` overrides it,
which is what a project spanning several repos needs. Run it from a plain
shell — herdr refuses to launch nested inside one of its own panes.

```bash
herdrh            # in ~/projects/personal/dotfiles → session "dotfiles"
herdrh lavoro     # explicit, for grouping several repos in one session
```
...
`prefix+q` detaches. For a file tree in a pane, run `broot` — its `br` wrapper
(which also changes the shell's directory on exit) is generated by
`broot --install`, not by this repo.

A workspace is the per-project container inside a session, and it can be
anchored anywhere:

```bash
herdr workspace create --cwd . --label <name>   # explicit --cwd always wins
```

Use one session per context you want isolated and one workspace per project
inside it: the agent sidebar and its priority ordering only span the session
you are attached to.
